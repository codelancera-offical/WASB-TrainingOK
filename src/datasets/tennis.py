...
class Tennis(object):
    def __init__(self, 
        ...
        # debug summary
        log.info('[Tennis] root_dir={} ext={} csv_filename={} frames_in={} frames_out={} step={}'.format(self._root_dir, self._ext, self._csv_filename, self._frames_in, self._frames_out, self._step))
        log.info('[Tennis] train_matches={} test_matches={}'.format(self._train_matches, self._test_matches))
    ...
    def _gen_seq_list(self, 
        ...
        for match in matches:
            ...
            if not osp.exists(match_clip_dir):
                log.warning('[Tennis] match dir not found: {}'.format(match_clip_dir))
                continue
            clip_names     = os.listdir(match_clip_dir)
            clip_names.sort()
            clip_names = clip_names[:int(len(clip_names)*num_clip_ratio)]
            num_rallies += len(clip_names)
            log.info('[Tennis] match={} clips_found_after_ratio={}'.format(match, len(clip_names)))
            for clip_name in clip_names:
                clip_seq_list    = []
                clip_seq_gt_dict = {}
                ...
                if not osp.exists(clip_frame_dir):
                    log.warning('[Tennis] clip dir not found: {}'.format(clip_frame_dir))
                    continue
                if not osp.exists(clip_csv_path):
                    log.warning('[Tennis] CSV not found: {}'.format(clip_csv_path))
                    continue
                ...
                for frame_name in os.listdir(clip_frame_dir):
                    if frame_name.endswith(self._ext):
                        frame_names.append(frame_name)
                frame_names.sort()
                if len(frame_names) < self._frames_in:
                    log.warning('[Tennis] too few frames (< frames_in): {} (frames={})'.format(clip_frame_dir, len(frame_names)))
                num_frames         += len(frame_names)
                num_frames_with_gt += len(ball_xyvs)
                log.info('[Tennis] clip={} frames={} csv_rows={} sequences_possible={}'.format(clip_name, len(frame_names), len(ball_xyvs), max(0, len(ball_xyvs)-self._frames_in+1)))
